...
| private    | chk  | 현재 점수까지의 답을 만들기 위해 사용한 원소의 위치는 1, 사용하지 않은 원소의 위치는 0으로 저장합니다. |
...
| public     | void ClearChk(void)                 | chk배열을 0으로 초기화합니다.                                |
| public     | void ClearChk(int i, int j)         | chk배열의 (i, j) 위치를 0으로 설정합니다.                    |
| public     | void SetChk(int i, int j)           | chk배열의 (i, j) 위치를 1로 설정합니다.                      |
| public     | void GetChk(int i, int j)           | chk배열의 (i, j) 위치의 값을 반환합니다.                     |
...
| 접근제어자 | 이름                                                         | 설명                                                         |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| private    | bool BackTracking(const int v[8][14], int i, int j, int now, int flag) | 이차원배열 v에서 now를 찾는 백트래킹을 수행합니다.<br />flag가 1인 경우 chk배열에 사용되는 원소를 기록합니다. |
| public     | bool HasNum(const int v[8][14], int now, int flag)           | 이차원배열 v에 now가 존재하는지 확인합니다.<br />BackTracking을 수행합니다.<br />flag가 1인 경우 chk배열에 사용되는 원소를 기록합니다. |
...
| public     | void Optimizer(Gene& gene, int max_iter, int flag)          | 이 유전자에 대해 지역 최적화를 최대 max_iter번 수행합니다.<br />만약 flag가 1인 경우에는 커팅 없이 수행하고, 0인 경우에는 커팅을 동반해 수행합니다.<br />flag가 0인 경우 수행 속도는 많이 빨라지지만, 최적화가 덜 되는 경우가 있습니다. |
...
| private    | void Optimize(void)                                      | 모든 유전자에 대해 GeneManager 클래스의 지역 최적화를 수행합니다.<br />10%의 확률로 커팅 없는 최적화를, 90%의 확률로 커팅을 동반한 최적화를 진행합니다. |
